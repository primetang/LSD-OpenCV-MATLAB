...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    plhs[0] = mxCreateDoubleMatrix(4, ntl->size, mxREAL);
    double *pos_mat = mxGetPr(plhs[0]);

    for (int j = 0; j != ntl->size ; ++j)
    {
        pos_mat[4 * j + 0] = ntl->values[0 + j * ntl->dim] + 1;
        pos_mat[4 * j + 2] = ntl->values[1 + j * ntl->dim] + 1;
        pos_mat[4 * j + 1] = ntl->values[2 + j * ntl->dim] + 1;
        pos_mat[4 * j + 3] = ntl->values[3 + j * ntl->dim] + 1;
    }
}